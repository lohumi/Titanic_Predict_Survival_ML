...
#combine=pd.concat([train_df,test_df],axis=1)
...
#Based on our assumptions and decisions we will drop the Cabin and Ticket features.
...
# drop name and passanger id column from dataset
train_df=train_df.drop(['Name', 'PassengerId'], axis=1)
test_df=test_df.drop(['Name'], axis=1)
combine = [train_df, test_df]

# convert categorical feature "Sex"
for ds in combine:
...
# drop FamilySize,SibSp and Parch features
...
#train_df[['Embarked']].isnull().sum() # 2 null values present
...
#test_df[['Fare']].isnull().sum() # single null value present
...
'''Our problem is a classification and regression problem.We are also perfoming a category of machine learning 
which is called supervised learning as we are training our model with a given dataset.
With these two criteria, we can narrow down our choice of models to a few
...
FinalResult.to_csv('../submission.csv')
